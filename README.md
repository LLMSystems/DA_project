...
一套圍繞**內政部戶政司門牌查詢**的端到端資料系統：自動化爬取門牌異動資料、
...
| [加分題](試題三/README.md#加分題-自動化排程) | 自動化排程 | Ofelia（Docker 原生） | 定時起新容器跑全量抓取，結果同進監控 |
...
對應試題「請提供」清單的示範畫面與檔案證據，彙整於 **[docs/README.md](docs/README.md)**。
...
| 系統架構圖 | [試題四](試題四/) |
| （加分）排程設計 | [試題三 — 自動化排程](試題三/README.md#加分題-自動化排程) |

## 設計重點

- **共用資料層解耦**：爬蟲寫、API 唯讀同一份 SQLite；所有服務寫同一份 Log，靠 named volume 串接，無直接程式相依。
- **驗證碼自動辨識**：ddddocr + Otsu 前處理 + 5 碼閘門 + 失敗自動重試/降級人工；進階可開多變體與 beam CTC 解碼（合併 holdout exact 達 85%，詳見[試題一 README](試題一/README.md)）。
- **平台偵測為主的異常通報**：應用只負責寫結構化 Log，由 Grafana 以 LogQL 統一偵測，新增告警規則免改程式。
- **冪等與閉環**：`INSERT OR IGNORE`（`row_hash`）去重，排程定期重跑不產生重複；排程結果一樣進監控、失敗一樣告警。
